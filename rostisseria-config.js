...
(function () {
  ...
  Promise.all([
    ...
    /* Pedimos la capacidad ANTES de inicializar el widget para evitar
       race conditions: si el usuario añade productos al carrito mientras
       slotDur todavía es el default, el pseudo-servicio se generaría con
       duración incorrecta. */
    ...
    var schedApi = results[2] || null;

    /* Fijar slotDur antes de cualquier renderizado del carrito */
    if (schedApi && schedApi.capacidad && schedApi.capacidad.duracion_slot_min) {
      slotDur = schedApi.capacidad.duracion_slot_min;
    }
